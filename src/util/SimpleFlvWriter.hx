...
class SimpleFlvWriter {
	...
	private function writeUI24(stream : Dynamic, p : UInt) : Void{
		var byte1 : Int = p >> 16;
	...
	private function writeUI16(stream : Dynamic, p : UInt) : Void{
		stream.writeByte(p >> 8);
		stream.writeByte(p & 0xff);
	}
	
	private function writeUI4_12(stream : Dynamic, p1 : UInt, p2 : UInt) : Void{
